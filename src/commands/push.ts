...
export async function pushCommand(): Promise<void> {
  console.log(chalk.cyan("\n🔐 envfy push - Encrypt and upload .env\n"));

  // Check if project is initialized
  if (!isProjectInitialized()) {
    console.log(chalk.red("✗ Project not initialized."));
    console.log(chalk.gray("  Run 'envfy init' first to set up your project."));
    process.exit(1);
  }
  ...
  if (!keyEntry) {
    ...
    console.log(
      chalk.gray(
        "  Run 'envfy pull' here once (with the shared passphrase) to set up this machine."
      )
    );
    process.exit(1);
  }
  ...
  console.log(chalk.white(`  1. Commit ${chalk.yellow(".envfy/config.json")} to your repo`));
  console.log(chalk.white(`  2. Share the ${chalk.yellow("passphrase")} securely (Signal, 1Password, etc.)`));
  console.log(chalk.white(`  3. Teammates run ${chalk.yellow("envfy pull")} and enter the passphrase`));
  console.log(chalk.green("\n" + "═".repeat(50) + "\n"));
}
